import { useEffect, useState } from "react";
import { useAdminService } from "../../services/admin.service";
import CreateStaffModal from "../../components/admin/CreateStaffModal";
import { motion } from "framer-motion";
import { Users } from "lucide-react";

interface Staff {
  _id: string;
  name: string;
  email: string;
  department: string;
  college: string;
  enrollment: string;
}

const ManageStaff = () => {
  const { getAllStaff } = useAdminService();

  const [open, setOpen] = useState(false);
  const [staffList, setStaffList] = useState<Staff[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetchStaff();
  }, []);

  const fetchStaff = async () => {
    try {
      setLoading(true);
      const res = await getAllStaff();
      setStaffList(res.data.data);
    } finally {
      setLoading(false);
    }
  };

  return (
    <motion.div
      initial={{ opacity: 0, y: 15 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.4 }}
      className="bg-slate-900/60 backdrop-blur-xl border border-slate-800 p-8 rounded-2xl shadow-lg flex flex-col h-[75vh]"
    >
      {/* Header */}
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-indigo-500/10 border border-indigo-500/30 flex items-center justify-center">
            <Users size={18} className="text-indigo-400" />
          </div>

          <h2 className="text-2xl font-semibold text-white">
            Staff Management
          </h2>
        </div>

        <button
          type="button"
          onClick={() => setOpen(true)}
          className="px-5 py-2 rounded-xl bg-gradient-to-r from-green-600 to-green-500 hover:opacity-90 text-white font-medium transition"
        >
          + Create Staff
        </button>
      </div>

      {/* Scrollable Staff List */}
      <div className="flex-1 overflow-y-auto pr-2 space-y-5">

        {loading ? (
          <p className="text-slate-400">Loading staff...</p>
        ) : staffList.length === 0 ? (
          <div className="text-center text-slate-400 py-10">
            No staff members found.
          </div>
        ) : (
          staffList.map((staff) => (
            <motion.div
              key={staff._id}
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              className="p-6 bg-slate-800/50 border border-slate-700 rounded-xl hover:border-indigo-500/40 transition"
            >
              <div className="flex justify-between items-center mb-2">
                <h3 className="text-lg font-semibold text-white">
                  {staff.name}
                </h3>

                <span className="text-xs px-3 py-1 rounded-full bg-indigo-500/10 text-indigo-400 border border-indigo-500/30">
                  STAFF
                </span>
              </div>

              <p className="text-sm text-slate-300">
                Email: {staff.email}
              </p>

              <p className="text-sm text-slate-300">
                Enrollment: {staff.enrollment}
              </p>

              <p className="text-sm text-slate-300">
                Department: {staff.department}
              </p>

              <p className="text-sm text-slate-400">
                College: {staff.college}
              </p>
            </motion.div>
          ))
        )}
      </div>

      {/* Create Staff Modal */}
      {open && (
        <CreateStaffModal
          onClose={() => {
            setOpen(false);
            fetchStaff(); //  auto refresh after create
          }}
        />
      )}
    </motion.div>
  );
};

export default ManageStaff;